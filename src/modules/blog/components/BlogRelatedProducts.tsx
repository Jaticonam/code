...
export default function BlogRelatedProducts({article}:{article:BlogArticle}){
  const {data:products=[]}=useProducts();

  const related=products.filter(p=>article.relatedProducts?.includes(p.id));
  ...
  return (
    <section className="blog-related-products">
      <div className="blog-related-products-head">
        <span><ShoppingBag size={16}/> Productos mencionados</span>
